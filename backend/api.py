...
def sanitize_mermaid(src: str) -> str:
    """Clean up Mermaid code so it's always renderable."""
    if not src:
        return ""
    s = src.strip()
    ...
    for line in s.splitlines():
        just = line.strip()
        just = re.sub(r';\s*$', '', just)  # remove trailing semicolons
        if just:
            lines.append(just)
    s = "\n".join(lines)
...
def azure_mcp(payload: dict = Body(...), _=Depends(require_api_key)):
    ...
    # Detect resources from diagram text for cost
    items = []
    ...
    cost = price_items(items) if items else {"currency": "USD", "total_estimate": 0, "items": []}
